...
describe('Sync Utils tests', () => {
	...
	describe('performBidirectionalSync', () => {
		test('Should import workspaces from Obsidian to Context Spaces', async () => {
		...
		test('Should create workspaces in Obsidian from Context Spaces', async () => {
		...
		test('Should handle name conflicts correctly', async () => {
			...
			// Context Spaces settings with different name
			const settings = {
				spaces: {
					[workspaceId]: {
						name: contextName,
	...
	describe('needsSync', () => {
		test('Should return true when sync is needed', () => {
			...
			// Add workspace to Obsidian but not to Context Spaces
