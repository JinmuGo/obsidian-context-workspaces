...
export default class ContextSpacesPlugin extends Plugin {
	...
	async onload() {
		...
		// Add settings tab
		this.addSettingTab(new ContextSpacesSettingTab(this.app, this));
	...
	async initializeDefaultSpace() {
		if (!isWorkspacesPluginEnabled(this.app)) {
			new Notice('Context Spaces requires the Workspaces plugin to be enabled.');
			return;
		}
	...
	/**
	 * Perform bidirectional synchronization between Context Spaces and Obsidian workspaces
	 */
			...
			if (deletedWorkspaces.length > 0) {
				...
				setTimeout(async () => {
					...
					if (currentWorkspaceDeleted) {
						...
						new Notice(
							`${deletedWorkspaces.length} workspaces were removed from Context Spaces.`,
							3000
						);
					}
				}, 0);
