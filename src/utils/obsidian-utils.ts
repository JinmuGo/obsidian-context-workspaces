...
export async function applySpaceTheme(
	...
	try {
		...
		if (hasChanges) {
			console.log('Context Spaces: Applied theme changes:', changes.join(', '));
		}
	} catch (error) {
...
/**
 * Monitor Obsidian's loadWorkspace API calls and automatically switch Context Spaces
 */
export function setupWorkspaceLoadMonitoring(app: App, plugin: ContextSpacesPluginLike): void {
	try {
		const workspaces = getWorkspacesPlugin(app);
		if (!workspaces?.enabled || !workspaces.instance) {
			return;
		...
		console.log('Context Spaces: Workspace load monitoring enabled');
	} catch (error) {
...
export function removeWorkspaceLoadMonitoring(app: App): void {
	try {
		...
		console.log('Context Spaces: Workspace load monitoring disabled');
	} catch (error) {
...
export function backupThemeState(app: App): void {
	try {
		...
		console.log('Context Spaces: Theme state backed up:', themeStateBackup);
	} catch (error) {
...
export async function restoreThemeState(app: App): Promise<void> {
	try {
		if (!themeStateBackup) {
			console.log('Context Spaces: No theme state backup to restore');
			return;
		}
		...
		if (changes.length > 0) {
			console.log('Context Spaces: Restored theme state:', changes.join(', '));
		}
...
export function clearThemeStateBackup(): void {
	themeStateBackup = null;
	console.log('Context Spaces: Theme state backup cleared');
}
