...
> {
	...
	render() {
		if (this.state.hasError) {
			return (
				this.props.fallback || (
					<div className="obsidian-context-spaces-error">
...
const SpaceItem: React.FC<SpaceItemProps> = ({
	...
	return (
		<button
			ref={setNodeRef}
			style={style}
			className={`obsidian-context-space-item ${isActive ? 'active' : ''} ${isCenter ? 'center' : ''} ${isDragging ? 'obsidian-context-spaces-dragging' : ''}`}
		...
		>
			...
			<div className="obsidian-context-space-icon" {...listeners}>
				{space.icon || '📄'}
			</div>

			{/* Auto-save indicator */}
			{space.autoSave && <div className="obsidian-context-space-auto-save">•</div>}
		</button>
	);
};
		...
		if (shouldUseCenteredLayout) {
			setTimeout(() => {
				const carousel = containerRef.current?.querySelector(
					'.obsidian-context-spaces-carousel'
				) as HTMLElement;
				const activeItem = carousel?.querySelector(
					'.obsidian-context-space-item.active'
				) as HTMLElement;
	...
	return (
		<ErrorBoundary>
			<DndProvider onDragEnd={handleDragEnd} items={spaceOrder}>
				<div ref={containerRef} className="obsidian-context-spaces-sidebar">
					{/* Header */}
					<div className="obsidian-context-spaces-header">
						<h4 className="obsidian-context-spaces-title">Context Spaces</h4>

						{/* Help button */}
						<button
							type="button"
							className="obsidian-context-spaces-help-btn"
							onClick={() => {
								new Notice(
						...
						<button
							type="button"
							className="obsidian-context-spaces-add-btn"
					...
					{/* Carousel container */}
					<div
						className={`obsidian-context-spaces-carousel-container ${shouldUseCenteredLayout ? 'centered-layout grid-carousel' : ''} ${animationDirection ? `animate-${animationDirection}` : ''}`}
					>
						{/* Left navigation button */}
						<button
							type="button"
							className={`obsidian-context-spaces-nav-btn left ${!canGoPrevious ? 'disabled' : ''}`}
						...
						<div className="obsidian-context-spaces-carousel">
						...
						<button
							type="button"
							className={`obsidian-context-spaces-nav-btn right ${!canGoNext ? 'disabled' : ''}`}
