...
export class SpaceCreateModal extends Modal {
	...
	onOpen() {
		...
		contentEl.addClass('obsidian-context-spaces-space-create-modal');

		// Set modal header
		this.titleEl.setText('Create New Space');

		// Modal content area
		const modalContent = contentEl.createDiv('obsidian-context-spaces-modal-content');

		// Icon and name input area
		const inputSection = modalContent.createDiv('obsidian-context-spaces-input-section');

		// Icon selection area
		const iconSection = inputSection.createDiv('obsidian-context-spaces-icon-section');
		const _iconLabel = iconSection.createEl('label', { text: 'Emoji' });
		this.iconButton = iconSection.createEl('button', {
			cls: 'obsidian-context-spaces-icon-picker-button',
			text: this.icon,
		});
		...
		// Name input area
		const nameSection = inputSection.createDiv('obsidian-context-spaces-name-section');
		...
		const themeSection = modalContent.createDiv('obsidian-context-spaces-theme-section');
		...
		const themeModeSection = modalContent.createDiv(
			'obsidian-context-spaces-theme-mode-section'
		);
		...
		const descriptionSection = modalContent.createDiv(
			'obsidian-context-spaces-description-section'
		);
		...
		const buttonContainer = contentEl.createDiv(
			'obsidian-context-spaces-modal-button-container'
		);
		...
		if (this.showEmojiPicker) {
			...
			if (iconContainer) {
				this.emojiPickerContainer = iconContainer.createDiv(
					'obsidian-context-spaces-emoji-picker-popup'
				);
