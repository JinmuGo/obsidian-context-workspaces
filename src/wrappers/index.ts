import { type App, Modal, Notice, type Plugin, PluginSettingTab } from 'obsidian';
import React from 'react';
import { ContextSpacesSettingTab as ReactContextSpacesSettingTab } from '../components/ContextSpacesSettingTab';
...
export class SpaceEditModal extends Modal {
	private plugin: ContextSpacesPlugin;
	private spaceId: string;
	private onSave?: () => void;
	private reactWrapper: ReactModalWrapper;

	constructor(app: App, plugin: ContextSpacesPlugin, spaceId: string, onSave?: () => void) {
		super(app);
		this.plugin = plugin;
...
export class SpaceManagerModal extends Modal {
	...
	constructor(app: App, plugin: ContextSpacesPlugin) {
		super(app);
		this.plugin = plugin;
		this.reactWrapper = new ReactModalWrapper();
	}
...
// Wrapper for ContextSpacesSettingTab
export class ContextSpacesSettingTab extends PluginSettingTab {
	private plugin: ContextSpacesPlugin;
	...
	display(): void {
		...
		this.reactWrapper.render(
			React.createElement(ReactContextSpacesSettingTab, {
				app: this.app,
				plugin: this.plugin,
...
export class SidebarManager {
	...
	constructor(plugin: Plugin) {
		this.plugin = plugin as unknown as ContextSpacesPlugin;
		this.reactWrapper = new ReactWrapper();
	}
	...
	initialize() {
		...
		try {
			...
			this.container = leftSidebarContainer.createDiv({
				cls: 'obsidian-context-spaces-sidebar',
			});
		...
		this.observer = new MutationObserver((mutations) => {
			...
			for (const mutation of mutations) {
				if (mutation.type === 'childList') {
					...
					// 		(node as Element).classList?.contains('obsidian-context-spaces-sidebar') &&
					// 		node !== this.container
					// 	) {
					// 		// 중복 사이드바 발견 - 제거
